...
class Left_menu {
    ...
    private function _get_sidebar_menu_items($type = "") {
        ...
        if ($this->ci->login_user->user_type == "staff" && $type !== "client_default") {
            ...
            $access_vehicle = get_array_value($permissions, "vehicle");
            ...
            if ($this->ci->login_user->is_admin || $access_client) {
                $sidebar_menu["clients"] = array("name" => "clients", "url" => "clients", "class" => "users");
            }
...
if (get_setting("module_documents")) {
    $sidebar_menu["clients_contrat"] = array(
        "name" => "clients_contrat", // Clé de langue pour "Clients"
        "url" => "clients_contrat",
        "class" => "users" // icône pour les clients
    );
   
}
if (get_setting("module_documents")) {
    $sidebar_menu["miseADispo"] = array(
        "name" => "miseADispo", // Clé de langue pour "Clients"
        "url" => "mise_a_dispo",
        "class" => "check-circle" // icône pour les clients
    );
   
}
 if (get_setting("module_documents")) {
    $sidebar_menu["transferts"] = array(
        "name" => "transferts", // Clé de langue pour "Clients"
        "url" => "transferts",
        "class" => "navigation" // icône pour les clients
    );}
// Si le module Contrats est actif
if (get_setting("module_documents")) {
    $sidebar_menu["contrats"] = array(
        "name" => "contrats", // Clé de langue pour "Contrats"
        "url" => "contrats",
        "class" => "file-text" // icône pour les contrats
    );
}
  
  if (get_setting("module_documents"))  {
         $sidebar_menu["voitures_contrat"] = array(
            "name" => "voitures_contrat", // Clé de langue pour "Flotte de Voitures"
            "url" => "voitures_contrat",
            "class" => "key"
        );
    }
...
// Module Documents
// Vous devrez créer le paramètre 'module_documents' dans la table settings
if (get_setting("module_documents")) {
    $sidebar_menu["chauffeur_documents"] = array(
        "name" => "documents",
        "url" => "chauffeur_documents",
        "class" => "file-text",
        "submenu" => array(
            array("name" => "document_list", "url" => "chauffeur_documents", "class" => "list")
        )
    );
}
// Module Paiements Chauffeurs
if (get_setting("module_chauffeur_payments")) {
    $sidebar_menu["chauffeur_payments"] = array(
        "name" => "chauffeur_payments",       // Clé de langue
        "url" => "chauffeur_payments",       // URL du contrôleur
        "class" => "dollar-sign",            // Icône Feather pour l'argent
        "submenu" => array(
            array("name" => "payments_list", "url" => "chauffeur_payments", "class" => "list")
        )
    );
}
// Dans app/Libraries/Left_menu.php

// Module Jawaz
if (get_setting("module_jawaz")) {
    $sidebar_menu["jawaz"] = array(
        "name" => "jawaz",
        "url" => "jawaz",
        "class" => "airplay",
        "submenu" => array(
            array("name" => "jawaz_list", "url" => "jawaz", "class" => "list")
        )
    );
}
             ...
             // Module Carburant
if (get_setting("module_carburant")) {
    $sidebar_menu["carburant"] = array(
        "name" => "carburant",
        "url" => "fuel_cards",
        "class" => "credit-card",
        "submenu" => array(
            array(
                "name" => "cartes_carburant", 
                "url" => "fuel_cards", 
                "class" => "credit-card"
            ),
            array(
            ...
            array(
                "name" => "cartes_expirant", 
                "url" => "fuel_cards/expiring_cards", 
                "class" => "alert-triangle"
            )
        )
    );
}
if (get_setting("module_locations")) {
    $sidebar_menu["locations"] = array(
        "name" => "locations",
        "url" => "locations",
        "class" => "key",
        "submenu" => array(
            array(
                "name" => "toutes_locations", 
                "url" => "locations", 
                "class" => "list"
            ),
            array(
            ...
            array(
                "name" => "nouvelle_location", 
                "url" => "locations/add", 
                "class" => "plus"
            ),
            array(
                "name" => "rapports_locations", 
                "url" => "locations/report", 
                "class" => "chart-line"
            ),
            array(
                "name" => "locations_aujourdhui", 
                "url" => "locations?statut=en_cours", 
                "class" => "calendar-day"
            )
        )
    );
}
if (get_setting("module_maintenance")) {
    $sidebar_menu["maintenance"] = array(
        "name" => "maintenance", // Clé de langue pour la traduction
        "url" => "maintenance",  // URL du contrôleur
        "class" => "tool",       // Icône Feather 'tool'
        "submenu" => array(
            array("name" => "maintenance_list", "url" => "maintenance", "class" => "list"),
            // Vous pourriez ajouter d'autres liens ici plus tard, comme des rapports.
        )
    );
}
            if ($this->ci->login_user->is_admin || $access_vehicle) {
    $sidebar_menu["vehicles"] = array(
        "name" => "vehicles", 
        "url" => "vehicles", 
        "class" => "truck",
        "submenu" => array(
            array("name" => "all_vehicles", "url" => "vehicles", "class" => "list"),
            array("name" => "add_vehicle", "url" => "vehicles/add", "class" => "plus-circle"),
            array("name" => "available_vehicles", "url" => "vehicles/available", "class" => "check-circle"),
            array("name" => "sold_vehicles", "url" => "vehicles/sold", "class" => "x-circle"),
            array("name" => "vehicle_statistics", "url" => "vehicles/statistics", "class" => "bar-chart-2")
        )
    );
}
