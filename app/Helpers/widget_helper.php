 ...
 */
 ...
 if (!function_exists('mise_a_dispo_recente_widget')) {
    function mise_a_dispo_recente_widget() {
        $db = \Config\Database::connect(); // Accès à la base de données

        // On utilise le Query Builder pour plus de sécurité
        $builder = $db->table('rise_mise_a_dispo');
        $builder->where('deleted', 0);
        $builder->orderBy('date_debut', 'DESC');
        $builder->limit(5); // On prend les 5 plus récentes
        $query = $builder->get();
        $mises_a_dispo = $query->getResult();

        // On construit le HTML du widget
        $html = '
        <div class="card">
            <div class="card-header">
                <i data-feather="truck" class="icon-16"></i>&nbsp;Mises à disposition récentes
            </div>
            <div class="list-group list-group-flush">
        ';

        if ($mises_a_dispo) {
            foreach ($mises_a_dispo as $mad) {
                $date = date('d/m/Y', strtotime($mad->date_debut));
                $html .= '<a href="#" class="list-group-item">' . htmlspecialchars($mad->nom_client) . '<span class="float-end">' . $date . '</span></a>';
            }
        } else {
            $html .= '<div class="card-body">Aucune mise à disposition trouvée.</div>';
        }
...
if (!function_exists('income_vs_expenses_widget')) {
    ...
    function income_vs_expenses_widget($custom_class = "", $show_own_expenses_only_user_id = 0) {
